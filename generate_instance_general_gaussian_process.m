function [ G,server,client,request, c_cache, c_link, rand_c_link,demand_SP ] = generate_instance_general_gaussian_process( n_timestamp,batch_num,A,cmin,cmax,cmin_s,cmax_s,C,skewness,deg_client,n_client, linkmin, linkmax, c_v, c_client, k_paths)
...
% m2 = readmatrix(['data/GP_top30_to_50_videos_model_plus_5_pred_11_29_' num2str(batch_num) '.csv']);%batch_num, GP_top30_videos_model_plus_5_pred_11_29_[1~20]; ref: https://ww2.mathworks.cn/help/matlab/ref/readmatrix.html
% when in topology = 50, we need the matrix from: GP_top30_to_50_videos_model_plus_5_pred_11_29_1
% request rates):
V = length(A);
% generate link costs:
G = zeros(size(A));
