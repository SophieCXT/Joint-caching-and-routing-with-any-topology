function [ cost, load, paths_output, links, cost_splittable, load_vector_splittable ] = bicriteria_MSUFP_gurobi( G, c_link, s, d, lambda, K )
...
[X,fval] = linprog_gurobi(f,A,b,Aeq,beq,lb,ub,options);% look at all the 
%input parameters that related to demand, step through at what point the 
%computation failed.
%check the lambda, normalized value of 1 request per unit time, original
%link capacity, meet the satisfactory.
